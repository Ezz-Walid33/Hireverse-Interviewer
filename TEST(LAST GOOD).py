...
def run_interview():
    ...
    for question in selected_behavioral:
        ...
        quickeval = behavioral_chain.predict(input=f"Was this answer interesting? Answer yes or no only: {candidate_followup}").lower()
        print(f"\EVALUATION: {quickeval} END OF EVALUATION")
        # Optional: Second follow-up if interesting
        if "yes" in quickeval:
            print(f'GOING INTO SECOND FOLLOWUP')
    ...
    for question_data in selected_technical:
        ...
        # Ask question
        response = technical_chain.predict(input=f"Ask this technical question: {question}. Do not mention any evaluation criteria.")
        print(f"\nInterviewer: {response}")
        candidate_answer = input("Candidate: ")
